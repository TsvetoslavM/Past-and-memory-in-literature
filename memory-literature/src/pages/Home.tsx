...
const sections = [
  ...
  {
    title: 'Видове памет',
    description: 'Открий различните форми на паметта и тяхното отражение в литературата.',
    icon: <PsychologyIcon fontSize="large" />, route: '/memory-types', color: '#43E97B'
  },
  {
    title: 'Литературни примери',
    description: 'Примери за паметта в българската и световната литература.',
              ...
              {[
                {
                  ...
                  answer: 'По време на войни и кризи. В големите епически романи за война (напр. „Война и мир" от Лев Толстой) героите често се обръщат към спомените за покоя и обикновения живот, за да компенсират шока от полетата на битка. Пиер Безухов, например, в размислите си за смисъла на живота, се връща към годините, когато семейният му дом е бил украсен за Коледа, търсейки утеха в паметта.',
                  conclusion: 'В много класически творби войната и кризите отключват у хората по-силна носталгия и спомени за изгубеното спокойствие, докато в мирните периоди паметта се превръща в средство за изследване на идентичността и укрепване на семейната и обществената история.'
                },
                {
                  question: 'Кои ценят спомените по-силно – тези, при които всичко е наред, или тези, които преживяват лишения и страдания?',
                  answer: 'Хората, които страдат и преживяват лишения. В редица романи с теми за глад, емиграция или социално неравенство (например „Над пропастта във житата" от Джером Д. Селинджър или „Над златния поток" от Харолд Муд) страдащите герои често придават на спомените почти митологичен статус.',
                  conclusion: 'В литературата често се среща възгледът, че тежките изпитания превръщат паметта в утеха и спасение. За хората в беда спомените често са по-ценни, защото идеализират детството, родния дом или изгубената близост.'
                },
                {
                  ...
                  conclusion: 'При лишените от сигурност паметта се превръща в морален ориентир, основа за оцеляване и начин да запазят човечността си.'
                },
                {
                  question: 'Как паметта за трагедията се отразява в литературните произведения?',
                  answer: 'Паметта за трагедиите в литературата често се развива в две посоки - документалната (съхраняване на историческата истина) и художествената (творческа интерпретация и метафоризация).',
                  conclusion: 'И двете форми показват колко трудно е да оставим болезнените спомени непроменени – те търсят израз и често се превръщат в изкуство.'
                }
              ].map((item, index) => (
                <Paper
                  key={index}
